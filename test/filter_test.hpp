...
namespace filter_test {
    ...
    int run() {
        ...
        using make_stride2_sequence = lift_<divide_by_two_celing,make_sequence_<lift_<multiply_by_2>>>;

        call_<make_sequence_<make_algo<make_stride2_sequence>>,extent>{} = call_<make_sequence_<make_algo<make_filtered_sequence>>,extent>{};


        //fusion
        std::pair<int,int>{} = pack_(false,'a',1,2,true) >>= filter_<is_<int>>{} | [](auto l, auto r){ return std::make_pair(l,r);};
        (void)(1 == (pack_(false,'a',1,true) >>= filter_<is_<int>>{} | [](auto l){ return l; }));
